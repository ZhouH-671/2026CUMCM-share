"""裕度（风险分位）搜索：先粗扫、再细扫、最后一维切片，结果写入 result/。
...
用法（在 question3-4 目录下）：
    python src/search.py --dry-run          # 只看搜索计划
    python src/search.py                    # 粗扫 + 细扫 + 切片，多进程
    python src/search.py --stage coarse     # 只跑粗扫
    python src/search.py --workers 8

结果逐条追加到 result/margin_search.csv，重复配置自动跳过，中断后可续跑。
"""
