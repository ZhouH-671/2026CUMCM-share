"""读取 result/margin_search.csv，画“裕度 → 费用”曲线并输出最优配置报告。

用法（在 question3-4 目录下）：
    python src/plot.py
...
def figure1(frame: pd.DataFrame, night: float, day: float) -> Path:
    ...
    path = RESULT_DIR / "fig1_margin_curves.png"
    fig.savefig(path)
    plt.close(fig)
    return path
...
def figure2(frame: pd.DataFrame, night: float, day: float) -> Path:
    ...
    fig.tight_layout()
    path = RESULT_DIR / "fig2_margin_heatmap.png"
    fig.savefig(path)
    plt.close(fig)
    return path
...
def figure3(frame: pd.DataFrame, night: float, day: float) -> Path:
    ...
    path = RESULT_DIR / "fig3_tradeoff.png"
    fig.savefig(path)
    plt.close(fig)
    return path
...
def write_report(frame: pd.DataFrame, night: float, day: float) -> Path:
    ...
    path = RESULT_DIR / "best_config.md"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path
...
def main() -> None:
    if not CSV_PATH.is_file():
        raise SystemExit("找不到 %s，请先运行 python src/search.py" % CSV_PATH)
    frame = load_frame()
    best = frame.iloc[0]
