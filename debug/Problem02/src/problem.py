...
输入：
    ../data/appendix-2nd.xlsx   第 1 页电价，第 2 页实际负载，第 3 页实际光伏
    ../result/pred.xlsx         第 1 页预测负载，第 2 页预测光伏
    comparison_load.py, comparison_pv.py   用于估计 1 月历史预测残差
...
# 允许 comparison_load.py 和 comparison_pv.py 与主脚本在同一目录，
# 或在 pred 子目录中
MODEL_DIR = (
    SCRIPT_DIR / "pred"
    if (SCRIPT_DIR / "pred" / "comparison_load.py").is_file()
    else SCRIPT_DIR
)
...
# 固定路径
DATA_PATH = Path("../data/appendix02.xlsx")
PRED_PATH = Path("../result/pred.xlsx")
OUTPUT_PATH = Path("../result/result2.xlsx")
...
def load_price(path: Path) -> np.ndarray:
    ...
    if tuple(rows[0][:2]) != ("时间", "电价"):
        raise ValueError("电价工作表表头不匹配，应为：时间 | 电价")
    ...
    if values.shape != (PERIODS_PER_DAY,) or not np.isfinite(values).all():
        raise ValueError("电价数据必须是 144 个有限数值")
    if (values < 0).any():
...
def load_actual_series(path: Path) -> tuple[list[date], np.ndarray, np.ndarray]:
    """读取 appendix02.xlsx 第2页实际负载和第3页实际光伏。"""
    workbook = openpyxl.load_workbook(path, read_only=True, data_only=True)
    try:
        worksheets = workbook.worksheets
        if len(worksheets) < 3:
            raise ValueError("appendix02.xlsx 至少需要 3 个工作表")

        # 第 2 页：实际小区负载
        # 第 3 页：实际光伏发电功率
        actual_sheets = [worksheets[1], worksheets[2]]
...
def main() -> None:
    ...
    # 电价：appendix-2nd.xlsx 第 1 页
    price = load_price(data_path)

    # 实际负载和实际光伏：appendix-2nd.xlsx 第 2、3 页
    ...
    if any(target not in actual_by_date for target in pred_dates):
        raise ValueError("pred.xlsx 中存在 appendix-2nd.xlsx 没有的日期")

    start_time = perf_counter()

    # 1 月暖启动预测：仍然用 appendix-2nd.xlsx 的实际数据构造
