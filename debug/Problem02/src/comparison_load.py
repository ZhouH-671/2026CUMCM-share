...
def load_history(path: Path) -> tuple[list[date], list[str], np.ndarray]:
    ...
    try:
        # appendix-2nd.xlsx 第 1 页不是负载数据；
        # 第 2 页（索引 1）对应附件2.xlsx 的第 1 页，即小区负载。
...
def main() -> None:
    ...
    output_dir = args.output_dir or root / "result" / "load"
    csv_path = output_dir / "forecast_load_5days.csv"
    save_forecast(csv_path, forecasts)
    plot_path = output_dir / f"comparison_load.svg"
