...
运行：
    python output/question2/question2.py

输入：
    pred.xlsx                 时间序列模型给出的 2025 年负载和光伏预测
    附件/附件1.xlsx            电价曲线
    附件/附件2.xlsx            实际负载和实际光伏
    comparison_load.py, comparison_pv.py          用于估计 1 月历史预测残差

输出：
    output/question2/result2.xlsx
    output/question2/model_R2_evaluation.csv
"""
...
def main() -> None:
    ...
    parser.add_argument(
        "--evaluation-output",
        type=Path,
        default=SCRIPT_DIR / "model_R2_evaluation.csv",
    )
    args = parser.parse_args()

    root = find_project_root(SCRIPT_DIR)
    pred_path = args.pred or (
        SCRIPT_DIR / "pred.xlsx" if (SCRIPT_DIR / "pred.xlsx").exists()
        else root / "question3-2" / "data" / "pred.xlsx"
    )
    output_path = args.output or SCRIPT_DIR / "result2.xlsx"
    price = load_price(root / "附件" / "附件1.xlsx")
