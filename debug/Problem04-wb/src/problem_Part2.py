"""问题四-3：波动电价下基于光伏预报的多阶段滚动购电—储能优化。

运行（在项目根目录下）：
    python src/problem4_3.py

输入：
    data/appendix02.xlsx                第 1 页实际负载，第 2 页实际光伏
    data/appendix03.xlsx                0:00、6:00、12:00、18:00 光伏预报
    data/appendix04.xlsx                实际电价，用于结算
    data/pred.xlsx                      第 1 页预测负载，第 2 页预测光伏
    result/forecast/pred_price_{method}.xlsx  预测电价，用于决策
    src/depend/question_2nd.py          数据读取、1 月暖启动
    src/depend/comparison_load.py       负载预测
    src/depend/comparison_pv.py         光伏预测

输出：
    result/result4-3_{method}.xlsx

说明：
    与问题三 problem.py 相比，仅修改数据来源与路径：
        - 决策阶段：用 prediction_HoltWinters.xlsx 的预测电价做 MILP；
        - 结算阶段：用 appendix04.xlsx 的实际电价结算计划购电、调整、紧急购电；
...
COMPARISON_DIR = RESULT_DIR / "forecast"

DATA_PATH = DATA_DIR / "appendix02.xlsx"
PV_FORECAST_PATH = DATA_DIR / "appendix03.xlsx"
ACTUAL_PRICE_PATH = DATA_DIR / "appendix04.xlsx"
PRED_PATH = DATA_DIR / "pred.xlsx"
PRICE_PRED_PATH = COMPARISON_DIR / "pred_price_holtwinters.xlsx"
OUTPUT_PATH = RESULT_DIR / "Part02" / "result4-3_holtwinters.xlsx"
...
def load_pv_forecasts(path: Path) -> dict[tuple[date, int], np.ndarray]:
    """读取 appendix03.xlsx 的四版小时级光伏预报。"""
    if not path.exists():
        raise FileNotFoundError(f"找不到光伏预报文件：{path}")
...
def solve_interval_milp(
    ...
    *,
    ...
    if result.x is None or result.status != 0:
        raise RuntimeError(f"问题四-3 区间 MILP 求解失败：{result.message}")
...
def solve_day(
    ...
    for decision_hour in (6, 12, 18):
        ...
        adjusted = solve_interval_milp(
            future_net,
            predicted_price[decision_index:],
            storage,
    ...
    adjustment_cost = float(
        np.sum(
            1.5 * actual_price * np.maximum(delta, 0.0)
            + 0.5 * actual_price * np.maximum(-delta, 0.0)
        )
    )
    plan_cost = float(actual_price @ plan.purchase_kwh)
    emergency_cost = float(5 * actual_price @ actual_emergency)
...
def write_result4_3(path: Path, results: list[DailyResult]) -> None:
    ...
    adjusted_sheet.append(
        ["日期\\时间", *time_headers, "全天调整购电量", "全天调整费用"]
    )
    for result in results:
        adjusted_sheet.append(
            [
...
def main() -> None:
    ...
    # 实际负载、实际光伏：appendix02.xlsx 第 1、2 页
    actual_dates, actual_load, actual_pv = q2.load_actual_series(DATA_PATH)
    print(f"[数据] 实际负载/光伏：{len(actual_dates)} 天，"
          f"{actual_dates[0]} 至 {actual_dates[-1]}")

    # 预测负载：1 月暖启动 + pred.xlsx
    _, load_forecasts = load_forecast_inputs(DATA_PATH, PRED_PATH)
    print(f"[数据] 负载预测：{len(load_forecasts)} 天")

    # 光伏预报：appendix03.xlsx
    pv_forecasts = load_pv_forecasts(PV_FORECAST_PATH)
    print(f"[数据] 光伏预报：{len(pv_forecasts)} 组")

    # 预测电价：prediction_HoltWinters.xlsx
    predicted_price_by_date = load_price_prediction(PRICE_PRED_PATH)
    print(f"[电价] 预测电价：{len(predicted_price_by_date)} 天，"
    ...
    print(f"[电价] 实际电价：{len(actual_price_by_date)} 天，"
          f"{min(actual_price_by_date)} 至 {max(actual_price_by_date)}")

    # 预测负载日期：pred.xlsx
    pred_dates, _, _ = q2.load_prediction_series(PRED_PATH)
    actual_by_date = {
        target: index for index, target in enumerate(actual_dates)
    }

    # 检查覆盖
    missing_load = [d for d in pred_dates if d not in load_forecasts]
    if missing_load:
        raise ValueError(
            f"负载预测缺少以下日期：{missing_load[:5]} ... "
            f"共 {len(missing_load)} 天"
        )
    missing_pred_price = [
        d for d in pred_dates if d not in predicted_price_by_date
    ]
    if missing_pred_price:
        raise ValueError(
            f"预测电价缺少以下日期：{missing_pred_price[:5]} ... "
            f"共 {len(missing_pred_price)} 天"
        )
    ...
    missing_pv = [
        ...
        if not all((d, h) in pv_forecasts for h in DECISION_HOURS)
    ]
    if missing_pv:
        raise ValueError(
            f"光伏预报缺少以下日期：{missing_pv[:5]} ... "
            f"共 {len(missing_pv)} 天"
        )
    ...
    # ========================================================
    # 第二段：2 月到 12 月，按 pred_dates 顺序滚动优化
    # ========================================================
    storage = SOC_INITIAL
    output_results: list[DailyResult] = []
    ...
    for target in pred_dates:
        ...
        result = solve_day(
            target,
            predicted_price_by_date[target],
            actual_price_by_date[target],
            base_load,
            pv_forecasts,
    ...
    write_result4_3(args.output, output_results)
    ...
    print(f"求解日期：{output_results[0].target_date} 至 "
          f"{output_results[-1].target_date}")
