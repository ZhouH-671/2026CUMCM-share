...
每天在 0:00 制定计划，在 6:00、12:00、18:00 根据最新光伏预报调整未来
购电计划，并在实际运行阶段处理储能和紧急购电。模型使用历史实际光伏订正
附件 3 的小时级预报，并恢复小时内的 10 分钟变化形状。

信息价值实验通过 decision_hours、oracle_hours 等参数复用本文件的运行逻辑。

运行（在项目根目录下）：
    python -m src.problem03.problem_new
    python -m src.problem03.problem_new \
        --quantile-overnight 0.60 --quantile-day 0.60
    python -m src.problem03.problem_new --decision-hours none
    python -m src.problem03.problem_new --decision-hours 3,6,12,18 --oracle-hours 3
...
from src.problem02 import problem as q2
...
DECISION_HOURS = (0, 6, 12, 18)          # 附件 3 的预报发布时刻（只用于校验/拟合）
ADJUST_HOURS_DEFAULT = (6, 12, 18)      # 默认的调整决策时刻
...
def solve_day(
    ...
    decision_hours: tuple = ADJUST_HOURS_DEFAULT,
    oracle_hours: frozenset = frozenset(),
    oracle_plan: bool = False,
) -> DailyResult:
    """完成一天的计划、若干次调整和实际运行。

    decision_hours  当天做调整决策的时刻，默认 6/12/18；
                    传空元组表示只有 0:00 一次计划决策、之后不再调整。
    oracle_hours    这些调整时刻直接使用当天实际光伏（“超能力”上界）；
                    新增时刻（附件 3 没有对应预报）必须放进这里。
    oracle_plan     True 时 0:00 计划直接使用当天实际光伏。
    """
    if oracle_plan:
        pv0 = np.asarray(actual_pv, dtype=float).copy()
    else:
    ...
    for decision_hour in decision_hours:
        ...
        if decision_hour in oracle_hours:
            pv_current = np.asarray(actual_pv, dtype=float).copy()
        else:
            pv_current = corrector.expand(
                corrector.forecasts[(target, decision_hour)],
                decision_hour,
                target,
            )
...
def load_inputs() -> dict:
    ...
    if not _INPUT_CACHE:
        price = q2.load_price(APPENDIX_DIR / "附件1.xlsx")
...
def run_simulation(
    ...
    for target in actual_dates:
        ...
        if oracle_plan or oracle_residual:
            forecast_pv0 = np.asarray(actual_pv[actual_index], dtype=float)
        else:
        ...
        if OUTPUT_START <= target <= OUTPUT_END:
            ...
            if collect_daily:
                daily_rows.append(
                    (
                        target,
                        float(result.plan_cost),
                        float(result.adjustment_cost),
                        float(result.emergency_cost),
                        float(
                            result.plan_cost
...
def residual_profile(
    ...
    """只统计“净负荷预报残差”，不求解任何 MILP，几秒钟即可跑完。

    残差 = 实际净负荷 − 0:00 预报净负荷，单位 kWh/时段。
    用来量化：不确定性有多大、分布在一天中的哪些时段、储能能否吸收。
    """
    inputs = load_inputs()
    actual_dates = inputs["actual_dates"]
    actual_load = inputs["actual_load"]
    ...
    for target in actual_dates:
        corrector.fit(target)
        index = actual_by_date[target]
        base_load = load_forecasts[target]
        pv0 = corrector.expand(pv_forecasts[(target, 0)], 0, target)
        actual_net = (actual_load[index] - actual_pv[index]) * DT
        forecast_net = (base_load - pv0) * DT
        rows.append(actual_net - forecast_net)
        corrector.observe(target, actual_pv[index])

    matrix = np.asarray(rows, dtype=float)
    keep = np.array(
        [OUTPUT_START <= day <= OUTPUT_END for day in actual_dates], dtype=bool
    )
    matrix = matrix[keep]

    blocks = {
        "0:00-6:00": slice(0, 36),
        "6:00-12:00": slice(36, 72),
        "12:00-18:00": slice(72, 108),
        "18:00-24:00": slice(108, 144),
    }
    block_stats = {}
    for name, chunk in blocks.items():
        part = matrix[:, chunk]
        block_stats[name] = {
            "mean_abs_per_slot": float(np.abs(part).mean()),
            "total_abs_kwh": float(np.abs(part).sum()),
            "std_per_slot": float(part.std(axis=0).mean()),
        }

    sigma = matrix.std(axis=0)
    battery_per_slot = POWER_LIMIT
    return {
        "days": int(matrix.shape[0]),
        "slots": int(matrix.shape[1]),
        "sigma_per_slot": float(sigma.mean()),
        "mean_abs_per_slot": float(np.abs(matrix).mean()),
        "battery_per_slot": float(battery_per_slot),
        "buffering_ratio": float(sigma.mean() / battery_per_slot),
        "blocks": block_stats,
        "matrix": matrix,
    }
...
def main() -> None:
    ...
    parser.add_argument(
        "--pv-oracle",
        action="store_true",
        help="超能力实验：当天实际光伏完全已知（计划与调整都用实际光伏）",
    )
    parser.add_argument(
        "--decision-hours",
        type=str,
        default="",
        help="调整决策时刻，逗号分隔，例如 '3,6,12,18'；传 'none' 表示不调整",
    )
    parser.add_argument(
        "--oracle-hours",
        type=str,
        default="",
        help="这些调整时刻直接用实际光伏，逗号分隔，例如 '9'",
    )
    args = parser.parse_args()

    if args.decision_hours.strip().lower() == "none":
        decision_hours: tuple = ()
    elif args.decision_hours.strip():
        decision_hours = tuple(
            int(part) for part in args.decision_hours.split(",") if part.strip()
        )
    else:
        decision_hours = ADJUST_HOURS_DEFAULT
    oracle_hours = frozenset(
        int(part) for part in args.oracle_hours.split(",") if part.strip()
    )
    if args.pv_oracle:
        oracle_hours = frozenset(decision_hours)
        oracle_plan = True
    else:
        oracle_plan = False
    ...
    if args.pv_oracle:
        print("模式：超能力 —— 当天实际光伏完全已知（计划与调整都使用实际光伏）")
    print(
        "调整决策时刻：%s；其中用实际光伏的时刻：%s"
        % (
            "无（只有 0:00 一次计划）" if not decision_hours else decision_hours,
            sorted(oracle_hours) or "无",
        )
    )
    ...
    print(f"求解日期：{OUTPUT_START} 至 {OUTPUT_END}")
