...
用法：
    python pv.py
    python pv.py --date 2026-01-03 --time 14:00
"""
...
RESULT_DIR = ROOT / "result" / "problem02" / "evaluate" / "pv"
...
def plot_comparison(
    ...
    *,
    ...
    axis.set_title(f"光伏：实际值与新旧方法对比（每天 {time_label}）", fontsize=15, pad=12)
    axis.set_xlabel("日期")
...
def main() -> None:
    ...
    csv_path = output_dir / "pv_forecast_5days.csv"
    save_forecast(csv_path, forecasts)
    plot_path = output_dir / f"pv4_{target_time.replace(':', '-')}_comparison.svg"
