...
用法：
    python load.py
    python load.py --date 2026-01-03 --time 14:00
"""
...
RESULT_DIR = ROOT / "result" / "problem02" / "evaluate" / "load"
...
def plot_comparison(
    ...
    *,
    ...
    axis.plot(
        ...
        color="#c44e52",
        linewidth=1.15,
        label=f"load 新方法（2月起 MAE={improved_mae:.1f} kW）",
    )
    axis.plot(
    ...
    axis.set_title(f"小区负载：实际值与新旧方法对比（每天 {time_label}）", fontsize=15, pad=12)
    axis.set_xlabel("日期")
...
def main() -> None:
    ...
    dates, time_labels, history = load_history(APPENDIX_DIR / "附件2.xlsx")
    ...
    csv_path = output_dir / "load_forecast_5days.csv"
    save_forecast(csv_path, forecasts)
    plot_path = output_dir / f"load_{target_time.replace(':', '-')}_comparison.png"
