...
def find_project_root(start: Path) -> Path:
    for candidate in (start, *start.parents):
        if (candidate / "data" / "appendix02.xlsx").is_file():
            return candidate
    raise FileNotFoundError(f"从 {start} 向上未找到“data/appendix02.xlsx”")
...
def plot_comparison(
    ...
    *,
    ...
    axis.plot(
        ...
        color="#1677b8",
        linewidth=1.15,
        label=f"pv 新方法（2月起 MAE={improved_mae:.1f} kW）",
    )
    axis.plot(
...
def main() -> None:
    ...
    dates, time_labels, history = load_history(root / "data" / "appendix02.xlsx")
    ...
    output_dir = args.output_dir or root / "result" / "pv"
    csv_path = output_dir / "forecast_pv_5days.csv"
    save_forecast(csv_path, forecasts)
    plot_path = output_dir / f"comparison_pv.svg"
    ...
    print(f"2月起 pv 新方法 MAE：{improved_mae:.4f} kW")
